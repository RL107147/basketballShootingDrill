...
class TableViewController4: UITableViewController {
    ...
    var assignments4: [String] = [] {
        didSet {
            defaults4.set(assignments4, forKey: "Assignment4")
        }
    }
    
    override func viewDidLoad() {
        super.viewDidLoad()
        if let savedAssignments = defaults4.object(forKey: "Assignment4") as? [String]{
            assignments4 = savedAssignments
        }
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return assignments4.count
    }
    
    
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "Cell", for: indexPath)
        cell.textLabel?.text = assignments4[indexPath.row]
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            assignments4.remove(at: indexPath.row)
    ...
    @IBAction func onAddButtonTapped(_ sender: UIBarButtonItem) {
        ...
        let append = UIAlertAction(title: "add", style: .default) {(action) in
            ...
            self.assignments4.append(assignment!)
            self.tableView.reloadData()
